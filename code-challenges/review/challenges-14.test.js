...
const toTitleCase = (arr) => {
  ...
  let newArr = [];
  arr.forEach(str => {
    let tempStr = str.charAt(0).toUpperCase();
    tempStr += str.slice(1);
    newArr.push(tempStr);
  });
  return newArr;
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(char => {
    if (char.name === 'Luke Skywalker') {
      lukeMass = parseInt(char.mass);
    }
    if (parseInt(char.mass) > lukeMass) {
      heavyChars.push(char.name);
    }
  });
  return heavyChars.join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  arr.sort((a, b) => a.property < b.property ? -1
...
const isSecure = (url) => {
  // Solution code here...
  return Boolean(url.includes('https://'));
};
