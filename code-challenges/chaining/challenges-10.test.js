...
const totalSum = input => {
  ...
  let totalSum = 0;
  input.forEach(arr => {
    totalSum += arr.reduce((totalCount, num) => {
      totalCount += num;
      return totalCount;
    }, 0);
  });
...
const divisibleByFiveTwoToThePower = input => {
  ...
  const newArr = [];
  input.forEach(arr => {
    const tempArr = arr.filter(num => {
      if (typeof num === 'number' && num % 5 === 0) {
        return num;
      }
    });
    const anotherArr = tempArr.map(num => Math.pow(2, num));
    newArr.push(anotherArr);
  });
  return newArr;
};
...
let findMaleAndFemale = data => {
  ...
  data.filter(char => {
    if (char.gender === 'male' || char.gender === 'female') {
      swarsChars.push(char.name);
    }
  });
  return swarsChars.join(' and ');
};
...
let findShortest = data => {
  // Solution code here...
  let tempShortyName = '';
  let tempShortyHeight = 10000;
  data.filter(char => {
    if (char.height < tempShortyHeight) {
      tempShortyName = char.name;
      tempShortyHeight = char.height;
    }
  });
  return tempShortyName;
};
